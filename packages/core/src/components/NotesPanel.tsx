...
function downloadFeedback() {
  ...
  if (mode === 'post') {
    ...
    if (url) {
      ...
      }).then(() => { clearFeedback(); showToast('Feedback posted ✓'); }).catch(() => showToast('Post failed'));
    }
    return;
  }
  ...
  if (mode === 'vscode') {
    ...
    if (cb) { cb(md); clearFeedback(); showToast('Sent to Copilot ✓'); }
    return;
  }
...
function sendToTerminal() {
  const md = buildFeedbackMarkdown();
  const cb = onTerminalCallback.value;
